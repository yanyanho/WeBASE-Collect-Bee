...
import com.webank.webasebee.parser.generated.bo.method.BAC003SetURIBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC003.setURIMethodCrawlerService", havingValue = "on")
public class BAC003SetURIMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC003SetURI Transaction");
		BAC003SetURIBO entity = new BAC003SetURIBO();
		...
		entity.set_uri(String.valueOf(params.get(0).getValue()));
		entity.set_id(BigIntegerUtils.toLong(params.get(1).getValue()));
		entity.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));	
		entity.setIdentifier("BAC003SetURI");			
		
		log.debug("end process BAC003SetURI Transaction");
		return entity;
	}
}
