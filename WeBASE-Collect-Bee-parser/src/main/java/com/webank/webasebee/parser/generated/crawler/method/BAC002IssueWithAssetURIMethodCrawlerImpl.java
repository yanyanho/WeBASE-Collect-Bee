...
import com.webank.webasebee.parser.generated.bo.method.BAC002IssueWithAssetURIBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC002.issueWithAssetURIMethodCrawlerService", havingValue = "on")
public class BAC002IssueWithAssetURIMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC002IssueWithAssetURI Transaction");
		BAC002IssueWithAssetURIBO entity = new BAC002IssueWithAssetURIBO();
		...
		entity.setTo(AddressUtils.bigIntegerToString(params.get(0).getValue()));
		entity.setAssetId(BigIntegerUtils.toLong(params.get(1).getValue()));
		entity.setAssetURI(String.valueOf(params.get(2).getValue()));
		...
		entity.setIdentifier("BAC002IssueWithAssetURI");			
		
		log.debug("end process BAC002IssueWithAssetURI Transaction");
		return entity;
	}
}
