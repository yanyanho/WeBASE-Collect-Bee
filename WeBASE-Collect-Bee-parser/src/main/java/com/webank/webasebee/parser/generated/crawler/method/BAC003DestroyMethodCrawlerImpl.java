...
import com.webank.webasebee.parser.generated.bo.method.BAC003DestroyBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC003.destroyMethodCrawlerService", havingValue = "on")
public class BAC003DestroyMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC003Destroy Transaction");
		BAC003DestroyBO entity = new BAC003DestroyBO();
		...
		entity.set_value(BigIntegerUtils.toLong(params.get(1).getValue()));
		...
		entity.setIdentifier("BAC003Destroy");			
		
		log.debug("end process BAC003Destroy Transaction");
		return entity;
	}
}
