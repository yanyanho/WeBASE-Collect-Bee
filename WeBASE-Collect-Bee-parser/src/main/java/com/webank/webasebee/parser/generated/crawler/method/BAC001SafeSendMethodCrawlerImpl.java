...
import com.webank.webasebee.parser.generated.bo.method.BAC001SafeSendBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC001.safeSendMethodCrawlerService", havingValue = "on")
public class BAC001SafeSendMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC001SafeSend Transaction");
		BAC001SafeSendBO entity = new BAC001SafeSendBO();
		...
		entity.setValue(BigIntegerUtils.toLong(params.get(1).getValue()));
		entity.setData(String.valueOf(params.get(2).getValue()));
		entity.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));	
		entity.setIdentifier("BAC001SafeSend");			
		
		log.debug("end process BAC001SafeSend Transaction");
		return entity;
	}
}
