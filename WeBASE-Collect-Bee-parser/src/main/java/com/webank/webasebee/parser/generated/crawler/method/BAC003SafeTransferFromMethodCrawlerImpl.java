...
import com.webank.webasebee.parser.generated.bo.method.BAC003SafeTransferFromBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC003.safeTransferFromMethodCrawlerService", havingValue = "on")
public class BAC003SafeTransferFromMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC003SafeTransferFrom Transaction");
		BAC003SafeTransferFromBO entity = new BAC003SafeTransferFromBO();
		...
		entity.set_id(BigIntegerUtils.toLong(params.get(2).getValue()));
		entity.set_value(BigIntegerUtils.toLong(params.get(3).getValue()));
		...
		entity.setIdentifier("BAC003SafeTransferFrom");			
		
		log.debug("end process BAC003SafeTransferFrom Transaction");
		return entity;
	}
}
