...
import com.webank.webasebee.parser.generated.bo.method.BAC004OperatorSendBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC004.operatorSendMethodCrawlerService", havingValue = "on")
public class BAC004OperatorSendMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC004OperatorSend Transaction");
		BAC004OperatorSendBO entity = new BAC004OperatorSendBO();
		...
		entity.setSender(AddressUtils.bigIntegerToString(params.get(0).getValue()));
		entity.setRecipient(AddressUtils.bigIntegerToString(params.get(1).getValue()));
		entity.setAmount(BigIntegerUtils.toLong(params.get(2).getValue()));
		entity.setData(BytesUtils.bytesArrayToString(params.get(3).getValue()));
		entity.setOperatorData(BytesUtils.bytesArrayToString(params.get(4).getValue()));
		entity.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));	
		entity.setIdentifier("BAC004OperatorSend");			
		
		log.debug("end process BAC004OperatorSend Transaction");
		return entity;
	}
}
