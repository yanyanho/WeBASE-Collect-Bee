...
import com.webank.webasebee.parser.generated.bo.method.BAC004SendBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC004.sendMethodCrawlerService", havingValue = "on")
public class BAC004SendMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC004Send Transaction");
		BAC004SendBO entity = new BAC004SendBO();
		...
		entity.setRecipient(AddressUtils.bigIntegerToString(params.get(0).getValue()));
		...
		entity.setIdentifier("BAC004Send");			
		
		log.debug("end process BAC004Send Transaction");
		return entity;
	}
}
