...
import com.webank.webasebee.parser.generated.bo.method.BAC003SetApprovalForAllBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC003.setApprovalForAllMethodCrawlerService", havingValue = "on")
public class BAC003SetApprovalForAllMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC003SetApprovalForAll Transaction");
		BAC003SetApprovalForAllBO entity = new BAC003SetApprovalForAllBO();
		...
		entity.set_operator(AddressUtils.bigIntegerToString(params.get(0).getValue()));
		entity.set_approved(BoolUtils.toBoolean(params.get(1).getValue()));
		entity.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));	
		entity.setIdentifier("BAC003SetApprovalForAll");			
		
		log.debug("end process BAC003SetApprovalForAll Transaction");
		return entity;
	}
}
