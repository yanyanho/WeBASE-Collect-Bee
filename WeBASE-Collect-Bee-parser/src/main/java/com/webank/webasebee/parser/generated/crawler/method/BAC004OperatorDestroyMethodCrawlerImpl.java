...
import com.webank.webasebee.parser.generated.bo.method.BAC004OperatorDestroyBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC004.operatorDestroyMethodCrawlerService", havingValue = "on")
public class BAC004OperatorDestroyMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC004OperatorDestroy Transaction");
		BAC004OperatorDestroyBO entity = new BAC004OperatorDestroyBO();
		...
		entity.setAccount(AddressUtils.bigIntegerToString(params.get(0).getValue()));
		entity.setAmount(BigIntegerUtils.toLong(params.get(1).getValue()));
		entity.setData(BytesUtils.bytesArrayToString(params.get(2).getValue()));
		entity.setOperatorData(BytesUtils.bytesArrayToString(params.get(3).getValue()));
		entity.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));	
		entity.setIdentifier("BAC004OperatorDestroy");			
		
		log.debug("end process BAC004OperatorDestroy Transaction");
		return entity;
	}
}
