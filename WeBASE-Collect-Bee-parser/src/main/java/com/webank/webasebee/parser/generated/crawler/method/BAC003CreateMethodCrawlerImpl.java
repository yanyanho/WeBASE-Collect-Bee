...
import com.webank.webasebee.parser.generated.bo.method.BAC003CreateBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC003.createMethodCrawlerService", havingValue = "on")
public class BAC003CreateMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC003Create Transaction");
		BAC003CreateBO entity = new BAC003CreateBO();
		...
		entity.set_initialSupply(BigIntegerUtils.toLong(params.get(0).getValue()));
		entity.set_uri(String.valueOf(params.get(1).getValue()));
		entity.set_data(BytesUtils.bytesArrayToString(params.get(2).getValue()));
		entity.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));	
		entity.setIdentifier("BAC003Create");			
		
		log.debug("end process BAC003Create Transaction");
		return entity;
	}
}
