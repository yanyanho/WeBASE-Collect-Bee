...
import com.webank.webasebee.parser.generated.bo.method.BAC005BAC005BO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC005.BAC005MethodCrawlerService", havingValue = "on")
public class BAC005BAC005MethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC005BAC005 Transaction");
		BAC005BAC005BO entity = new BAC005BAC005BO();
		...
		entity.setDescription(String.valueOf(params.get(0).getValue()));
		entity.setShortName(String.valueOf(params.get(1).getValue()));
		entity.setDefaultOperators(String.valueOf(params.get(2).getValue()));
		entity.setRegisterAddress(AddressUtils.bigIntegerToString(params.get(3).getValue()));
		entity.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));	
		entity.setIdentifier("BAC005BAC005");			
		
		log.debug("end process BAC005BAC005 Transaction");
		return entity;
	}
}
