...
import com.webank.webasebee.parser.generated.bo.method.BAC001SafeSendFromBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC001.safeSendFromMethodCrawlerService", havingValue = "on")
public class BAC001SafeSendFromMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC001SafeSendFrom Transaction");
		BAC001SafeSendFromBO entity = new BAC001SafeSendFromBO();
		...
		entity.setValue(BigIntegerUtils.toLong(params.get(2).getValue()));
		entity.setData(String.valueOf(params.get(3).getValue()));
		entity.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));	
		entity.setIdentifier("BAC001SafeSendFrom");			
		
		log.debug("end process BAC001SafeSendFrom Transaction");
		return entity;
	}
}
