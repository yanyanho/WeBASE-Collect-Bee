...
import com.webank.webasebee.parser.generated.bo.method.BAC002DestroyBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC002.destroyMethodCrawlerService", havingValue = "on")
public class BAC002DestroyMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC002Destroy Transaction");
		BAC002DestroyBO entity = new BAC002DestroyBO();
		...
		entity.setAssetId(BigIntegerUtils.toLong(params.get(0).getValue()));
		entity.setData(BytesUtils.bytesArrayToString(params.get(1).getValue()));
		entity.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));	
		entity.setIdentifier("BAC002Destroy");			
		
		log.debug("end process BAC002Destroy Transaction");
		return entity;
	}
}
