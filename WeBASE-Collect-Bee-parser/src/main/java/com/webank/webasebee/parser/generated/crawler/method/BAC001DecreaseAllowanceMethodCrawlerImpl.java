...
import com.webank.webasebee.parser.generated.bo.method.BAC001DecreaseAllowanceBO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC001.decreaseAllowanceMethodCrawlerService", havingValue = "on")
public class BAC001DecreaseAllowanceMethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC001DecreaseAllowance Transaction");
		BAC001DecreaseAllowanceBO entity = new BAC001DecreaseAllowanceBO();
		...
		entity.setSpender(AddressUtils.bigIntegerToString(params.get(0).getValue()));
		entity.setSubtractedValue(BigIntegerUtils.toLong(params.get(1).getValue()));
		entity.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));	
		entity.setIdentifier("BAC001DecreaseAllowance");			
		
		log.debug("end process BAC001DecreaseAllowance Transaction");
		return entity;
	}
}
