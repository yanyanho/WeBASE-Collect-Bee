...
import org.fisco.bcos.temp.BAC005.IssuerRemovedEventResponse;
import com.webank.webasebee.common.bo.data.EventBO;
import com.webank.webasebee.parser.crawler.face.BcosEventCrawlerInterface;
import com.webank.webasebee.parser.generated.bo.event.BAC005IssuerRemovedBO;
...
@Service
@Slf4j
@ConditionalOnProperty(name = "monitor.BAC005.IssuerRemovedCrawlerService", havingValue = "on")
public class BAC005IssuerRemovedCrawlerImpl implements BcosEventCrawlerInterface {
	@Autowired
    private Web3j web3j;
    @Autowired
	...
	public List<EventBO> handleReceipt(TransactionReceipt receipt, BigInteger blockTimeStamp) {
		...
		for (IssuerRemovedEventResponse IssuerRemovedEventResponse : IssuerRemovedEventResponseList) {
			BAC005IssuerRemovedBO bAC005IssuerRemoved = new BAC005IssuerRemovedBO();
			bAC005IssuerRemoved.setIdentifier("BAC005IssuerRemoved");		
			bAC005IssuerRemoved.setBlockHeight(receipt.getBlockNumber().longValue());
			bAC005IssuerRemoved.setEventContractAddress(receipt.getContractAddress());
			bAC005IssuerRemoved.setTxHash(receipt.getTransactionHash());
			bAC005IssuerRemoved.setAccount(String.valueOf(IssuerRemovedEventResponse.account));
			bAC005IssuerRemoved.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));
			log.debug("depot IssuerRemoved:{}", bAC005IssuerRemoved.toString());
			list.add(bAC005IssuerRemoved);
		}
		return list;
	}
}
