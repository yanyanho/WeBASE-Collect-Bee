...
import com.webank.webasebee.parser.generated.bo.method.BAC001BAC001BO;

@Slf4j
@Service
@ConditionalOnProperty(name = "monitor.BAC001.BAC001MethodCrawlerService", havingValue = "on")
public class BAC001BAC001MethodCrawlerImpl implements BcosMethodCrawlerInterface {
	...
	public MethodBO transactionHandler(Transaction transaction, BigInteger blockTimeStamp,
	 Map.Entry<String, String> entry, String methodName, Map<String, String> txHashContractAddress) {
		log.debug("Begin process BAC001BAC001 Transaction");
		BAC001BAC001BO entity = new BAC001BAC001BO();
		...
		entity.setMinUnit(BigIntegerUtils.toInteger(params.get(2).getValue()));
		entity.setTotalAmount(BigIntegerUtils.toLong(params.get(3).getValue()));
		entity.setBlockTimeStamp(new Date(blockTimeStamp.longValue()));	
		entity.setIdentifier("BAC001BAC001");			
		
		log.debug("end process BAC001BAC001 Transaction");
		return entity;
	}
}
